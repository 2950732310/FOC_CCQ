...
/* DMA发送超时计数器（用于异常恢复） */
static volatile uint32_t dma_tx_start_tick = 0;
...
/**
 * @brief  等待上一次DMA传输完成（带超时-简单循环等待）
 */
static void UART_WaitDMAReady(void)
{
    uint32_t timeout = 1000000; /* 轮询超时计数 */
    while (dma_tx_busy)
    {
        if (--timeout == 0)
        {
 ...
 */
int UART_Printf_DMA(const char *fmt, ...)
{
    if(dma_tx_busy)
    {
        return -1;   // 直接丢弃
