...
static uint8_t led_brightness = 255U;   // 亮度缩放，0~255
static volatile uint8_t ws2812_dma_busy = 0U;

/**
 * @brief 设置 WS2812 亮度
 *
 * @param brightness 亮度值，范围 0~255，255 表示满亮，0 表示熄灭
 */
...
void RGB_ToggleGreen(void)
{
    ...
    if (led_on)
    {
        RGB_DisplayColorById(GREEN_F);         
    }
    else
    {
        RGB_DisplayColorById(BLACK_F);         
    }
}
...
void RGB_SetColor(uint8_t LedId, RGB_Color_TypeDef Color)
{
    ...
    uint8_t r = (uint16_t)Color.R * led_brightness / 255U;
    uint8_t g = (uint16_t)Color.G * led_brightness / 255U;
    uint8_t b = (uint16_t)Color.B * led_brightness / 255U;

    // 将颜色数据转换为 PWM 信号格式，存储到 led_buf 中
    for (uint8_t i = 0; i < 8; i++)
    {
        // 设置 G（绿色）分量
        led_buf[LedId][i]      = (g & (1 << (7 - i))) ? CODE_1 : CODE_0;
        // 设置 R（红色）分量
        led_buf[LedId][8 + i]  = (r & (1 << (7 - i))) ? CODE_1 : CODE_0;
        // 设置 B（蓝色）分量
        led_buf[LedId][16 + i] = (b & (1 << (7 - i))) ? CODE_1 : CODE_0;
    }
}
